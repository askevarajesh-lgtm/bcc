...
const apiCodeTemplate = `import { useState, useEffect, useCallback } from 'react';
...
`;
fs.writeFileSync('e:/Bcc Seo/frontend/src/api/userApi.js', userApi);

const companyApi = apiCodeTemplate + `
export const useGetCompaniesDropdownQuery = createQueryHook((params) => ({ url: '/brands/dropdown', params }));
`;
fs.writeFileSync('e:/Bcc Seo/frontend/src/api/companyApi.js', companyApi);

const integrationApi = apiCodeTemplate + `
export const useGetIntegrationsQuery = createQueryHook(() => '/integrations');
`;
fs.writeFileSync('e:/Bcc Seo/frontend/src/api/integrationApi.js', integrationApi);

const clientApi = apiCodeTemplate + `
export const useGetClientsQuery = createQueryHook((params) => ({ url: '/brands', params }));
`;
fs.writeFileSync('e:/Bcc Seo/frontend/src/api/clientApi.js', clientApi);
