...
const TaskCalendarView = ({ onTaskClick, departmentFilter }) => {
  ...
  const scheduledCountsByDate = useMemo(() => {
    ...
    for (let i = 1; i <= days; i++) {
      ...
      map[key] = filteredTasks.filter((task) =>
        taskScheduledForKanbanDay(task, day),
      ).length;
    }
    return map;
  ...
  const scheduledForDetail = useMemo(() => {
    return flatTasks.filter((task) =>
      taskScheduledForKanbanDay(task, detailDay),
    );
  }, [filteredTasks, detailDay]);

  const completedForDetail = useMemo(() => {
    return flatTasks.filter((task) => taskCompletedOnDay(task, detailDay));
  }, [filteredTasks, detailDay]);
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        className={`task-notes-modal task-notes-modal--composer ${
          isDark ? "task-notes-modal--dark" : "task-notes-modal--light"
        }`}
        onCancel={() => {
          setIsCreateNoteModalOpen(false);
          noteForm.resetFields();
      ...
      <Modal
        ...
        className={`task-notes-modal task-notes-modal--list ${
          isDark ? "task-notes-modal--dark" : "task-notes-modal--light"
        }`}
        footer={null}
